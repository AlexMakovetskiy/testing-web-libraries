...
import { ICard } from "../types/ComponentProps";

function Card({ item }: ICard) {
    return (
        <div className="max-w-full bg-white dark:bg-gray-800" >
            {
                item.featured !== null && item.feature_image !== undefined && item.feature_image !== null ? 
                <Link href={`/read/${item.slug}`}>
                    <Image className="rounded-lg p-3" width={1000} height={324} src="" alt="dfgdfg" />
                </Link> : " "
            }
            <div className="p-3">
